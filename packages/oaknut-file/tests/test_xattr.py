...
class TestWriteAcornXattrs:
    def test_write_basic(self, host_file):
        write_acorn_xattrs(host_file, load_address=0x1900, exec_address=0x8023)
    ...
    def test_write_overwrites(self, host_file):
        write_acorn_xattrs(host_file, load_address=0x1900, exec_address=0x8023)
        write_acorn_xattrs(host_file, load_address=0x2000, exec_address=0x3000)
        meta = read_acorn_xattrs(host_file)
...
class TestReadAcornXattrs:
    ...
    def test_acorn_takes_precedence_over_econet(self, host_file):
        ...
        write_econet_xattrs(host_file, load_address=0x1111, exec_address=0x2222, attr=0x17)
        write_acorn_xattrs(host_file, load_address=0x1900, exec_address=0x8023, attr=0x03)
        meta = read_acorn_xattrs(host_file)
        assert meta.load_address == 0x1900
...
class TestWriteEconetXattrs:
    def test_write_basic(self, host_file):
        write_econet_xattrs(host_file, load_address=0xFFFFDD00, exec_address=0xFFFFDD00)
...
class TestRoundTrip:
    def test_acorn_round_trip(self, host_file):
        write_acorn_xattrs(host_file, load_address=0x1900, exec_address=0x8023, attr=0x0B)
        meta = read_acorn_xattrs(host_file)
        assert meta.load_address == 0x1900
        assert meta.exec_address == 0x8023
        assert meta.access == 0x0B
    ...
    def test_access_flags_round_trip(self, host_file):
        ...
        write_acorn_xattrs(host_file, load_address=0x1900, exec_address=0, attr=int(flags))
        meta = read_acorn_xattrs(host_file)
        assert Access(meta.access) == flags
