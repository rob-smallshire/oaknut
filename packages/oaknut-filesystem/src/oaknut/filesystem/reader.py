...
class ImageReader:
    ...
    def window(self, offset: int, length: int) -> "ImageReader":
        ...
        return ImageReader(self._data, base=self._base + start, length=end - start)
...
def reader_for(source: ImageSource, *, suffix_hint: str | None = None) -> ImageReader:
    ...
    or a filesystem path. Path sources are memory-mapped read-only so a
    large hard-disc image is not read wholesale; the mapping is released
