...
class TestAddressWidth:
    ...
    def test_stat_adfs_shows_eight_hex_digits(self, runner: CliRunner, adfs_image_filepath: Path):
        out = _run(
            runner,
            "stat",
            "--as",
    ...
    def test_get_load_adfs_is_eight_hex_digits(self, runner: CliRunner, adfs_image_filepath: Path):
        out = _run(runner, "get-load", "--as", "display", f"{adfs_image_filepath}:$.Hello").output
        assert "0x00001900" in out

    def test_get_load_dfs_is_six_hex_digits(self, runner: CliRunner, dfs_image_filepath: Path):
        out = _run(runner, "get-load", "--as", "display", f"{dfs_image_filepath}:$.Hello").output
