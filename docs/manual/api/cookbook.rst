...
.. note::
   ...
   The recipes below are starter content carried over from the previous
   documentation. They are illustrative — they have not yet been moved
   into the runnable-example pipeline that captures their output at
   build time. That conversion lands as part of the next docs pass; see
   the README's example pipeline under ``scripts/readme-examples/`` for
   the pattern.
