...
- **All three transports (AUN, Piconet, HAT)** plug into the `oaknut.econet.transport` axis, each CI-tested via a shipped fake plus the shared conformance suite.
- Application layer **(started)** — `oaknut-econet-station` (the `Station` service host + the `Service` extension axis `oaknut.econet.service`, with direct and by-name plug-in registration) and `oaknut-econet-kvstore` (a working `KvServer` plug-in + `KvClient`) prove a full client↔server round-trip — request → port-dispatch → reply — over paired transports, entirely in memory.
- Full workspace suite green (3535 tests, 4 hardware-gated skips).
